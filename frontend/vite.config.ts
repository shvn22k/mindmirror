...
/** FastAPI inference service (uvicorn backend.main:app) — proxied under /inference in dev/preview */
const inferenceProxyTarget =
	process.env.INFERENCE_PROXY_TARGET ||
	process.env.VITE_INFERENCE_PROXY_TARGET ||
	"http://127.0.0.1:8000";
...
const inferenceProxy = {
	"/inference": {
		...
		changeOrigin: true,
		rewrite: (p: string) => p.replace(/^\/inference/, ""),
	},
} as const;
